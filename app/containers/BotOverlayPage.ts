...
function mapStateToProps(state: stateType) {
  return {
    ...
    alreadyAppliedFlatIds: Object.entries(state.cache.applications).filter(([flatId, application]) => application.reason !== 'UNSUITABLE').map(([flatId]) => flatId),
    unsuitableFlatIds: Object.entries(state.cache.applications).filter(([flatId, application]) => application.reason === 'UNSUITABLE').map(([flatId]) => flatId)
  };
}
