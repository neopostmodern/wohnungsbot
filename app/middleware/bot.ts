...
import { RESET_BOT, SET_BROWSER_VIEW_READY, SET_VERDICT } from "../constants/actionTypes";
...
import { discardApplicationProcess, endApplicationProcess, generateApplicationTextAndSubmit } from "../actions/application";
  ...
  const handlePuppetReady = async () => {
    ...
    if ((await electronUtils.evaluate('document.title')).includes('Ich bin kein Roboter')) {
      await electronUtils.humanInteraction(async () => {
        return (await electronUtils.evaluate('document.title')).includes('Ich bin kein Roboter');
      });
      await sleep(5000);
    ...
    if (puppet.url.startsWith('https://www.immobilienscout24.de/Suche')) {
      ...
      if (experimentalFeatures.sortByNewest && !puppet.url.includes('sorting=2')) {
        store.dispatch(setBotMessage('Sortierung ändern...️'));
        setImmediate(async () => {
          await electronUtils.evaluate("window.location = window.location + '&sorting=2'");
        });
        await sleep(2000);
  ...
  if (action.type === SET_VERDICT) {
    ...
    switch (verdict.action) {
      case FLAT_ACTION.NOTIFY_VIEWING_DATE:
        store.dispatch(sendFlatViewingNotificationMail(contactData, flatOverview));
        break;
