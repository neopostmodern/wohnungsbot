...
export default ((store: Store) => (next: (action: Action) => void) => async (action: Action) => {
  if (action.type === LOGIN || action.type === LOGOUT) {
    ...
    await store.dispatch(electronRouting('puppet', 'https://www.immobilienscout24.de/'));
    await sleep(4000);
    const {
      webContents
    ...
    if (action.type === LOGOUT || useAccount === USEACCOUNT.NEIN) {
      const {
        abortableAction: abortablePerformLogout,
        abort
      } = abortable(performLogout);
      AbortionSystem.registerAbort(abort);

      try {
        await timeout(abortablePerformLogout(store.dispatch, electronUtils, store.getState().configuration), 300000);
      } catch (error) {
      ...
      const {
        abortableAction: abortablePerformAutomaticLogin,
        abort
      } = abortable(performAutomaticLogin);
      AbortionSystem.registerAbort(abort);

      try {
        await timeout(abortablePerformAutomaticLogin(store.dispatch, electronUtils, store.getState().configuration), 300000);
      } catch (error) {
        // eslint-disable-next-line no-console
      ...
      const {
        abortableAction: abortablePerformManualLogin,
        abort
      } = abortable(performManualLogin);
      AbortionSystem.registerAbort(abort);

      try {
        await timeout(abortablePerformManualLogin(store.dispatch, electronUtils, store.getState().configuration), 300000);
      } catch (error) {
        // eslint-disable-next-line no-console
        console.error(`Error during login: ${error}`);
        AbortionSystem.abort(ABORTION_ERROR);
