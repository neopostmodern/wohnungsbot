...
export function targetedAction<T>(type: string, target: targetType, action: (parameters: T) => Action) {
...
export default ((target: targetType) => (store: Store) => (next: Dispatch) => (action: Action) => {
