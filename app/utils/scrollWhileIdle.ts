...
export default function scrollWhileIdle(getState: GetState, dispatch: Dispatch) {
  let alive = true;
  let direction = 1;
  ...
  const scroll = async () => {
    ...
    if (direction > 0 && (await electronUtils.evaluate(`window.scrollY + window.innerHeight > document.body.scrollHeight * 0.95`))) {
      direction = -1;
    } else if (direction < 0 && (await electronUtils.evaluate(`window.scrollY < 50`))) {
      direction = 1;
    }
