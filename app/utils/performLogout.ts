...
export default function* performLogout(dispatch: Dispatch, electronUtils: ElectronUtils) {
  ...
  if (yield electronUtils.elementExists('.topnavigation__sso-login--logged-in')) {
    ...
    yield sleep(1000);
    yield dispatch(electronRouting('puppet', 'https://sso.immobilienscout24.de/sso/logout'));
    yield sleep(3000);
  }
