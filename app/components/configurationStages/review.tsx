...
const reviewStage: StageDescription = {
  ...
  body: ({
    ...
    toggleBoolean
      ...
      <br />
      <input type={'checkbox'} checked={configuration.policies.autostart} onChange={() => toggleBoolean('policies.autostart')} />{' '}
      ...
      <div className={styles.row}>
        <div className={styles.column}>
          ...
          {configuration.additionalInformation.animals && configuration.additionalInformation.animals !== 'Keine' ? <>mit {configuration.additionalInformation.animals}</> : <>ohne Tiere</>}
          <br />
          <br />
          Einzug{' '}
          <span style={{
          ...
          {configuration.filter.maximumRent ? <>maximal {configuration.filter.maximumRent}€</> : <i>beliebig teurer</i>}{' '}
          Kaltmiete
          <br />
          {configuration.filter.minimumArea ? <>mindestens {configuration.filter.minimumArea}m²</> : <i>beliebig großer</i>}{' '}
          Wohnfläche
          <br />
          {configuration.filter.maximumRentPerSquareMeter ? <>maximal {configuration.filter.maximumRentPerSquareMeter}€/m²</> : <i>beliebiger</i>}{' '}
          Quadratmeterpreis (kalt)
          <br />
          <br />
          {configuration.filter.minimumRooms || configuration.filter.maximumRooms ? <>
          ...
          <br />
          im{' '}
          {configuration.filter.floors.slice().reverse().map(floor => floorToName(floor, 4)).join(', ')}
          <br />
          <br />
        ...
        <div className={styles.column}>
          <h3>...und schreibt...</h3>
          <ApplicationTextPreviews applicationText={configuration.applicationText} className={styles.applicationTextReview} />
          <h3>Der Bot wird dabei...</h3>
          {configuration.policies.fillAsLittleAsPossible ? <em>möglichst wenig</em> : <em>alle</em>}{' '}
