...
const RecentApplication = ({
  ...
  const street = addressParts.length > 1 ? addressParts[0] : <div className={styles.unknownStreet}>Straße nicht angegeben</div>;
  return <div className={`${styles.entry} ${success ? '' : styles.failure}`}>
      ...
      <div className={styles.entryBody}>
        ...
        <div className={`${styles.symbol} ${styles.clickable}`}>
          {pdfPath ? <span className="material-icons" onClick={openPDF} title="Wohnung ansehen (PDF, immer verfügbar)">
              picture_as_pdf
            </span> : null}
        </div>
        <div className={`${styles.symbol}`}>
          <a href={flatPageUrl(flatId)} target="_blank" rel="noopener noreferrer" title="Wohnung ansehen (im Browser, falls noch online)">
