...
import type { configurationBoolean, configurationNumbers } from "../reducers/configuration";
...
import type { FlexibleNode, StageDescription } from "./configurationStages/types";
type Props = {
  nextStage: () => void;
  previousStage: () => void;
...
export default class Configuration extends Component<Props> {
  ...
  handleKeyDown(event: KeyboardEvent) {
    ...
    if (event.target === document.body && event.key === 'Enter' && this.checkStageValid().stageValid) {
      this.goToNext();
    }
  }
  ...
  checkStageValid(): {
    ...
    if (stage.buttons.forward.checkInvalid) {
      const validationResult = stage.buttons.forward.checkInvalid(configuration);
  ...
  render() {
    ...
    return <div className={`${styles.wrapper} ${stage.rootContainer ? stage.rootContainer.className : ''}`} data-tid="container" id="configurationContainer">
        <nav className={styles.navigation}>
          ...
          <div className={styles.header}>
            {configuration.stage > 1 && configuration.stage < stages.length - 1 ? <>
                Schritt {configuration.stage - 1} von {stages.length - 3}
              </> : <>&nbsp;</>}
            {stage.title ? <h2>{this.renderAmbiguous(stage.title)}</h2> : null}
            {stage.subtitle ? <div>{this.renderAmbiguous(stage.subtitle)}</div> : null}
          </div>

          <div className={styles.buttonContainer} style={{
          textAlign: 'right'
        }}>
            <button type="button" onClick={this.goToNext} className={`primary ${stage.buttons.forward.className || ''}`} style={stage.buttons.forward.style} disabled={!stageValid}>
              <div className={styles.buttonIcon}>
        ...
        <div className={styles.container}>
          <div className={`${styles.innerContainer} ${stage.container ? stage.container.className || '' : ''}`}>
