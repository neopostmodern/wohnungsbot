...
export function clickAction(selector: string, {
  ...
  return async (dispatch: Dispatch) => {
    ...
    const boundingRect = await new ElectronUtils(webContents).getBoundingBox(selector, shadowRootSelector);
...
export async function scrollIntoView(webContents: WebContents, selector: string, {
  strategy = 'center',
  smooth = true,
  elementExistenceGuaranteed = true,
  shadowRootSelector
  ...
  while (AbortionSystem.nestedFunctionsMayContinue) {
    await electronUtils.evaluate(`${ElectronUtils.generateSelector(selector, shadowRootSelector)}.scrollIntoView({ behavior: ${smooth ? "'smooth'" : "'auto'"}, block: '${strategy}'})`);
...
export function scrollIntoViewAction(name: BrowserViewName, selector: string, strategy: ScrollIntoViewStrategy = 'center', smooth: boolean = true) {
  return async () => {
    const {
      webContents
...
export async function scrollIntoViewByPolicy(webContents: WebContents, selector: string, {
  scrollIntoViewPolicy = 'auto',
  overrideStrategy,
  elementExistenceGuaranteed = true,
  shadowRootSelector
