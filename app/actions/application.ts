...
import { popFlatFromQueue, returnToSearchPage, setBotIsActing, setBotMessage, setShowOverlay, taskFinished } from "./bot";
...
import AbortionSystem, { ABORTION_ERROR, ABORTION_MANUAL } from "../utils/abortionSystem";
export const generateApplicationTextAndSubmit = (flatId: string): ThunkAction => async (dispatch: Dispatch, getState: GetState) => {
  const {
    configuration,
    data
  }: {
  ...
  const {
    abortableAction: abortablePerformApplication,
    abort
  ...
  try {
    await timeout(abortablePerformApplication(dispatch, electronUtils, configuration, data.overview[flatId]), 300000);
    success = true;
  } catch (error) {
...
const markApplicationCompleted = async (dispatch: Dispatch, applicationData: ApplicationData) => {
  dispatch(popFlatFromQueue(applicationData.flatId));
  return dispatch(markCompleted(CACHE_NAMES.APPLICATIONS, applicationData.flatId, applicationData));
};
...
export const discardApplicationProcess = (flatOverview: OverviewDataEntry): ThunkAction => async (dispatch: Dispatch) => {
