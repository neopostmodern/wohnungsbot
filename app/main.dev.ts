...
import { addView, setAvailableVersion, setUpdateDownloadProgress, setWindow } from "./actions/electron";
...
.then(store => {
  ...
  const installExtensions = async () => {
    ...
    return Promise.all(extensions.map(name => installer.default(installer[name], forceDownload))).catch(error => {
  ...
  app.on('ready', async () => {
    ...
    const newView = (name: BrowserViewName, options: BrowserViewConstructorOptions, initialUrl: string): BrowserView => {
    ...
    const puppetView = newView('puppet', {
      webPreferences: {
        sandbox: true,
        contextIsolation: true,
        enableRemoteModule: false
    ...
    const configurationView = newView('configuration', {
      webPreferences: {
        nodeIntegration: true,
        contextIsolation: false,
        preload: `${__dirname}/preload.js`
